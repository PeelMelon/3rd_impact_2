...
<%
    ...
    try (Connection conn = DBManager.getDBConnection()) {
        ...
        try (PreparedStatement updatePstmt = conn.prepareStatement(updateViewsSql)) {
            updatePstmt.setString(1, seq);
            updatePstmt.executeUpdate();
        }
        
        // 동영상 정보 가져오기
        String selectSql = "SELECT v.seq, v.title, v.description, v.tags, v.thumbnail_url, v.video_id, v.views, COALESCE(vl.likes, 0) as likes, COALESCE(vl.dislike, 0) as dislike, v.logo_url, v.uploader_id, v.upload_date " +
                           "FROM video v LEFT JOIN video_like vl ON v.seq = vl.seq WHERE v.seq = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(selectSql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    video.put("video_id", rs.getString("video_id"));
                    video.put("views", rs.getString("views"));
                    video.put("likes", rs.getString("likes"));
                    video.put("dislike", rs.getString("dislike"));
    ...
    <div class="video_selection">
        ...
        <div class="allVideo">
            <div class="videoAndNext">
                <div class="player">
                    <div id="videoPlayer" data-video-id="<%= video.get("video_id") %>"></div>
                </div>
                        ...
                        <ul class="actions">
                            ...
                            <li>
                                <button id="dislikeButton" data-seq="<%= video.get("seq") %>" data-action="dislike">
                                    <i class="fas fa-thumbs-down"></i><span id="dislikeCount"><%= video.get("dislike") %></span>
                                </button>
                            </li>
                            <li>
